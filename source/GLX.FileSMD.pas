...
procedure TgxSMDVectorFile.LoadFromStream(aStream: TStream);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        frame.Position.Add(GLX.Utils.StrToFloatDef(tl[1]),
          StrToFloatDef(tl[2]), GLX.Utils.StrToFloatDef(tl[3]));
        v := AffineVectorMake(GLX.Utils.StrToFloatDef(tl[4]),
          StrToFloatDef(tl[5]), GLX.Utils.StrToFloatDef(tl[6]));
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                boneIDs[j].Weight := GLX.Utils.StrToFloatDef(tl[11 + j * 2]);
              end;

              nVert := FindOrAdd(boneIDs,
                AffineVectorMake(GLX.Utils.StrToFloatDef(tl[1]),
                StrToFloatDef(tl[2]), GLX.Utils.StrToFloatDef(tl[3])),
                AffineVectorMake(GLX.Utils.StrToFloatDef(tl[4]),
                StrToFloatDef(tl[5]),
                StrToFloatDef(tl[6])));
              nTex := TexCoords.FindOrAdd
                (AffineVectorMake(GLX.Utils.StrToFloatDef(tl[7]),
...
procedure TgxSMDVectorFile.SaveToStream(aStream: TStream);
var
  str, nodes: TStrings;
  i, j, k, l, b: Integer;
...
RegisterVectorFileFormat('smd', 'Half-Life SMD files', TgxSMDVectorFile);
