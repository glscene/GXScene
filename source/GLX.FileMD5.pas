...
procedure TgxMD5VectorFile.LoadFromStream(aStream: TStream);
  ...
  begin
    ...
    pos.X := GLX.Utils.StrToFloatDef(FTempString[2]);
    pos.Y := GLX.Utils.StrToFloatDef(FTempString[4]);
    pos.Z := GLX.Utils.StrToFloatDef(FTempString[3]);

    quat := QuaternionMakeFromImag(GLX.Utils.StrToFloatDef(FTempString[5]),
      GLX.Utils.StrToFloatDef(FTempString[7]),
      GLX.Utils.StrToFloatDef(FTempString[6]));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            mesh.TexCoords[vnum] :=
              AffineVectorMake(GLX.Utils.StrToFloatDef(FTempString[2]),
              1 - GLX.Utils.StrToFloatDef(FTempString[3]), 0);
        ...
        begin
          ...
          begin
            ...
            VertexWeight[wnum] := GLX.Utils.StrToFloatDef(FTempString[3]);
            VertexWeighted[wnum] :=
              AffineVectorMake(GLX.Utils.StrToFloatDef(FTempString[4]),
              GLX.Utils.StrToFloatDef(FTempString[6]),
              GLX.Utils.StrToFloatDef(FTempString[5]));
          end;
        end;
      end;
    until temp = '}';
  ...
  begin
    ...
    repeat
      ...
      if FTempString.Count >= 6 then
      begin
        pos := AffineVectorMake(GLX.Utils.StrToFloatDef(FTempString[0]),
          GLX.Utils.StrToFloatDef(FTempString[1]),
          GLX.Utils.StrToFloatDef(FTempString[2]));
        quat := QuaternionMakeFromImag(GLX.Utils.StrToFloatDef(FTempString[3]),
          GLX.Utils.StrToFloatDef(FTempString[4]),
          GLX.Utils.StrToFloatDef(FTempString[5]));
        FFramePositions.Add(pos);
        FFrameQuaternions.Add(quat);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          pos.X := GLX.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then
        begin
          pos.Y := GLX.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then
        begin
          pos.Z := GLX.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then
        begin
          quat.ImagPart.X := GLX.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then
        begin
          quat.ImagPart.Y := GLX.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart.Z := GLX.Utils.StrToFloatDef(FTempString[j]);
      end;
