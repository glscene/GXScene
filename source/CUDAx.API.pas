...
type
  ...
  private
    ...
    FCompiler: TgxSCUDACompiler;
    procedure SetCode(const Value: TStringList);
    procedure SetCompiler(const Value: TgxSCUDACompiler);
  ...
  protected
    ...
    FGLContextHandle: TgxVirtualHandle;
    FMapCounter: Integer;
    function GetIsAllocated: Boolean; override;
    procedure OnGLHandleAllocate(Sender: TgxVirtualHandle;
      var Handle: Cardinal);
    procedure OnGLHandleDestroy(Sender: TgxVirtualHandle; var Handle: Cardinal);
  ...
  public
    ...
    procedure CopyFrom(const AGLImage: TgxBitmap32); overload;
  ...
  private
    ...
    procedure SetDevice(const Value: TgxSCUDADevice);
...
procedure TgxSCUDA.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TgxSCUDA.SetDevice(const Value: TgxSCUDADevice);
begin
...
procedure TgxSCUDA.SetOnOpenGLInteropInit(AEvent: TOnOpenGLInteropInit);
begin
  FOnOpenGLInteropInit := AEvent;
  CuNotifyChange(cuchContext);
end;

function TgxSCUDA.GetContext: TCUDAContext;
begin
...
function TgxSCUDA.GetIsAllocated: Boolean;
begin
  Result := FContext.IsValid;
end;

function TgxSCUDA.GetModule(const i: Integer): TCUDAModule;
var
  j, k: Integer;
begin
  Result := nil;
...
procedure TCUDAModule.SetCompiler(const Value: TgxSCUDACompiler);
begin
...
begin
  if Assigned(FMaster) and (FMaster is TgxSCUDA) then
    Result := TgxSCUDA(FMaster).Context
  else
  begin
    Result := nil;
    ShowMessage(Format('Invalid master of module "%s"', [Name]));
    Abort;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage(Format('%s.LoadFromFile: file extension must be ptx or cubin',
        [Self.ClassName]));
  end
  else
...
begin
  if Self is TgxSCUDA then
    Result := TgxSCUDA(Self).Context
  else
    Result := TgxSCUDA(FMaster).Context;
end;
...
begin
  ...
  begin
    ShowMessage(strWrongParamSetup);
    Abort;
  end;
...
begin
  ...
  begin
    ShowMessage(Format(strFuncNotConnected, [Self.ClassName]));
    Abort;
  end;
  ...
  if FStatus <> CUDA_SUCCESS then
  begin
    ShowMessage(Format(strLaunchFailed, [Self.Name]));
    Abort;
  end;
end;
...
begin
  ...
  begin
    ShowMessage(Format(strFailUnmap, [Name]));
    Abort;
  end;
...
procedure TCUDAMemData.CopyTo(const AGLImage: TgxBitmap32);
var
  copyParam2D: TCUDA_MEMCPY2D;
  // copyParam3D: TCUDA_MEMCPY3D;
begin
...
procedure TCUDAMemData.CopyFrom(const AGLImage: TgxBitmap32);
var
  copyParam2D: TCUDA_MEMCPY2D;
  // copyParam3D: TCUDA_MEMCPY3D;
begin
...
begin
  if FMapping then
  begin
    ShowMessage(Format(strFailMap, [Name]));
    Abort;
  end;
...
procedure TCUDAGraphicResource.OnGLHandleAllocate(Sender: TgxVirtualHandle;
  var Handle: Cardinal);
begin
...
procedure TCUDAGraphicResource.OnGLHandleDestroy(Sender: TgxVirtualHandle;
  var Handle: Cardinal);
begin
  DestroyHandles;
end;
