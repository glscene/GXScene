    ...
    try
      ...
      begin
        ...
        for k := 0 to vals.Count - 1 do
          if vals[k] <> '' then
            list.Add(GLX.Utils.StrToFloatDef(vals[k], 0));
        Inc(idx);
      end;
      Result := idx;
    finally
    ...
    try
      ...
      begin
        ...
        if vals.Count >= 3 then
          list.Add(GLX.Utils.StrToFloatDef(vals[0], 0),
            GLX.Utils.StrToFloatDef(vals[1], 0),
            GLX.Utils.StrToFloatDef(vals[2], 0));
        Inc(idx);
      end;
      Result := idx;
    finally
