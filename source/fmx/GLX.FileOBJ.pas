...
(**** TOBJFGVertexNormalTexIndexList ****
  - based on TFGVertexNormalTexIndexList (GLX.VectorFileObjects.pas)
...
begin
  ...
  try
    ...
    begin
      ...
      for i := 0 to 3 do
        if sl.Count > i then
          Result.V[i] := GLX.Utils.StrToFloatDef(sl[i], 0)
        else
          break;
    end
    else
...
GLX.VectorFileObjects.RegisterVectorFileFormat('obj', 'WaveFront model file', TgxOBJVectorFile);
GLX.VectorFileObjects.RegisterVectorFileFormat('objf', 'Stripe model file', TgxOBJVectorFile);
