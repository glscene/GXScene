...
type
  ...
  TgxHeightTileInfo = packed record
    left, top, width, height: Integer;
    min, max, average: SmallInt;
    fileOffset: Int64; // offset to tile data in the file
  end;

  PgxHeightTileInfo = ^TgxHeightTileInfo;
  PPHeightTileInfo = ^PgxHeightTileInfo;

  TgxHeightTile = packed record
    info: TgxHeightTileInfo;
    data: array of SmallInt;
  end;

  PHeightTile = ^TgxHeightTile;
...
type
  ...
  TgxHeightTileFile = class(TObject)
  private
    FFile: TStream;
    FHeader: THTFHeader;
    FTileIndex: packed array of TgxHeightTileInfo;
    ...
    FQuadTable: array [0 .. cHTFQuadTableSize, 0 .. cHTFQuadTableSize]
      of array of Integer;
    FCreating: Boolean;
    FHeightTile: TgxHeightTile;
    FInBuf: array of ShortInt;
  protected
    function GetTiles(index: Integer): PgxHeightTileInfo;
  ...
  public
    ...
    function XYTileInfo(anX, anY: Integer): PgxHeightTileInfo;
    ...
    property Tiles[index: Integer]: PgxHeightTileInfo read GetTiles;
    function IndexOfTile(aTile: PgxHeightTileInfo): Integer;
...
constructor TgxHeightTileFile.CreateNew(const fileName: String;
  aSizeX, aSizeY, aTileSize: Integer);
begin
  with FHeader do
  begin
...
constructor TgxHeightTileFile.Create(const fileName: String);
var
...
begin
  ...
  FFile.Read(FTileIndex[0], SizeOf(TgxHeightTileInfo) * n);
...
destructor TgxHeightTileFile.Destroy;
var
  n: Integer;
begin
  if FCreating then
  begin
    ...
    n := Length(FTileIndex);
    FFile.Write(n, 4);
    FFile.Write(FTileIndex[0], SizeOf(TgxHeightTileInfo) * n);
    // write data size
    FFile.Position := 0;
    FFile.Write(FHeader, SizeOf(FHeader));
  end;
...
function TgxHeightTileFile.QuadTableX(x: Integer): Integer;
begin
  Result := ((x * (cHTFQuadTableSize + 1)) div (SizeX + 1)) and
    cHTFQuadTableSize;
end;

function TgxHeightTileFile.QuadTableY(y: Integer): Integer;
begin
  Result := ((y * (cHTFQuadTableSize + 1)) div (SizeY + 1)) and
    cHTFQuadTableSize;
end;

procedure TgxHeightTileFile.PackTile(aWidth, aHeight: Integer;
  src: PSmallIntArray);
var
  packWidth: Integer;
...
procedure TgxHeightTileFile.UnPackTile(source: PShortIntArray);
var
...
function TgxHeightTileFile.GetTileIndex(aLeft, aTop: Integer): Integer;
var
  I, key, n: Integer;
  p: PIntegerArray;
begin
...
function TgxHeightTileFile.GetTile(aLeft, aTop: Integer;
  pTileInfo: PPHeightTileInfo = nil): PHeightTile;
var
  I, n: Integer;
  tileInfo: PgxHeightTileInfo;
begin
  with FHeightTile.info do
    if (left = aLeft) and (top = aTop) then
    begin
...
procedure TgxHeightTileFile.CompressTile(aLeft, aTop, aWidth, aHeight: Integer;
  aData: PSmallIntArray);
begin
...
procedure TgxHeightTileFile.ExtractRow(x, y, len: Integer;
  dest: PSmallIntArray);
var
  rx: Integer;
  n: Cardinal;
...
function TgxHeightTileFile.XYTileInfo(anX, anY: Integer): PgxHeightTileInfo;
var
  tileList: TList;
begin
  ...
  try
    ...
    if tileList.count > 0 then
      Result := PgxHeightTileInfo(tileList.First)
    else
      Result := nil;
  finally
    tileList.Free;
...
function TgxHeightTileFile.XYHeight(anX, anY: Integer): SmallInt;
var
  tileInfo: PgxHeightTileInfo;
  tile: PHeightTile;
begin
...
procedure TgxHeightTileFile.TilesInRect(aLeft, aTop, aRight, aBottom: Integer;
  destList: TList);
var
...
function TgxHeightTileFile.TileCount: Integer;
begin
  Result := Length(FTileIndex);
end;

function TgxHeightTileFile.GetTiles(index: Integer): PgxHeightTileInfo;
begin
  Result := @FTileIndex[index];
end;

function TgxHeightTileFile.IndexOfTile(aTile: PgxHeightTileInfo): Integer;
var
  c: Cardinal;
begin
  c := Cardinal(aTile) - Cardinal(@FTileIndex[0]);
  if (c mod SizeOf(TgxHeightTileInfo)) = 0 then
  begin
    Result := (c div SizeOf(TgxHeightTileInfo));
...
function TgxHeightTileFile.TileCompressedSize(tileIndex: Integer): Integer;
begin
