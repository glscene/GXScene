...
type
  TgxProcTextureNoise = class(TgxTextureImage)
  private
    FNoiseMap: TgxBitmap32;
...
destructor TgxProcTextureNoise.Destroy;
begin
  ReleaseBitmap32;
  inherited;
end;

procedure TgxProcTextureNoise.UpdateNoise;
var
  X, Y, C: Integer;
...
function TgxProcTextureNoise.GetBitmap32: TgxBitmap32;
begin
  if not Assigned(FNoiseMap) then
  begin
    FNoiseMap := TgxBitmap32.Create;
...
procedure TgxProcTextureNoise.SetSeamless(const val: Boolean);
begin
...
procedure TgxProcTextureNoise.LoadFromFile(const fileName: string);
begin
  Assert(False, 'TgxProcTextureNoise.LoadFromFile not implemented');
end;

procedure TgxProcTextureNoise.ReleaseBitmap32;
begin
...
function TgxProcTextureNoise.GetWidth: Integer;
begin
  Result := FWidth;
end;

function TgxProcTextureNoise.GetDepth: Integer;
begin
  Result := 1;
end;
...
procedure TgxProcTextureNoise.SetHeight(const val: Integer);
begin
  if val <> FHeight then
  begin
    FHeight := val;
...
procedure TgxProcTextureNoise.SetWidth(const val: Integer);
begin
  if val <> FWidth then
  begin
    FWidth := val;
...
procedure TgxProcTextureNoise.SetMinCut(const val: Byte);
begin
  if val <> FMinCut then
  begin
    FMinCut := val;
...
procedure TgxProcTextureNoise.SetNoiseSharpness(const val: Single);
begin
...
procedure TgxProcTextureNoise.SetNoiseRandSeed(const val: Longint);
var
  i: Integer;
  seedBackup: Longint;
  z, r, theta: Single;
...
procedure TgxProcTextureNoise.Assign(Source: TPersistent);
begin
  if Assigned(Source) and (Source is TgxProcTextureNoise) then
  begin
    FWidth := TgxProcTextureNoise(Source).FWidth;
    FHeight := TgxProcTextureNoise(Source).FHeight;
    FMinCut := TgxProcTextureNoise(Source).FMinCut;
    FNoiseSharpness := TgxProcTextureNoise(Source).FNoiseSharpness;
    FNoiseRandSeed := TgxProcTextureNoise(Source).FNoiseRandSeed;
    Invalidate;
  end
  else
    inherited;
...
procedure TgxProcTextureNoise.NoiseAnimate(speed: Single);
begin
  FNoiseAnimate := FNoiseAnimate + speed;
  Invalidate;
end;

function TgxProcTextureNoise.Noise(x, y: Single): Single;
var
  ix, iy, iz: Integer;
...
procedure TgxProcTextureNoise.SetPermFromData(inPERM: array of Byte);
var
  I: Integer;
begin
  for I := 0 to 255 do
...
procedure TgxProcTextureNoise.SetPermToDefault;
begin
