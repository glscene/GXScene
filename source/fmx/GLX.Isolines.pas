...
type
  ...
  PVXIsoline = ^TgxIsoline;
  TgxIsoline = class (TObject)
    NP: Integer;
    Line: PVertex2DArr;
  ...
  TgxIsolineState = (ilsEmpty, ilsCalculating, ilsReady);

  TgxIsolines = class(TgxLines)
  public
    IsoVertex: TAffineVector;
    SpaceTextSF: array of TgxSpaceText;
...
function GetNextIsoline(var Isoline: TgxIsoline): Boolean;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Isoline := TgxIsoline.Create(np1 + np2);
            for k := 0 to np2 - 1 do
            begin
...
constructor TgxIsolines.Create(AOwner: TComponent);
begin
...
destructor TgxIsolines.Destroy;
begin
  FreeList;
  Nodes.Free;
  inherited;
...
procedure TgxIsolines.FreeList;
var
  i: integer;
begin
 if LineList<>nil then
  begin
    ...
    begin
      TgxIsoline(LineList.Items[i]).Free;
    end;
    LineList.Clear;
    IsolineState := ilsEmpty;
  end;
...
procedure TgxIsolines.MakeIsolines(var Depths: TMatrixArr; bmSize: integer;
  StartDepth, EndDepth: Single; Interval: Integer);
var
  Isoline: TgxIsoline;
...
constructor TgxIsoline.Create(LineSize: Integer);
begin
  inherited Create;
  NP := LineSize;
...
procedure TgxIsolines.Conrec(PlaneSFindex: Integer; PlaneSF: TgxfreeForm; Data:
              ...
              begin // ask if there a decision available ---+if if not(Deside=0)
                ...
                if ODD(K) then
                 ...
                 if (not IUniqueList.Contains(HgtL[K])) and
                   ...
                   begin
                     SpaceTextSF[K].Free;
                     SpaceTextSF[K]:= Tgxspacetext.CreateAsChild(self);
