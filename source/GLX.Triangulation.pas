...
constructor TgxDelaunay2D.Create;
begin
...
destructor TgxDelaunay2D.Destroy;
begin
  SetLength(Vertex, 0);
...
function TgxDelaunay2D.InCircle(Xp, Yp, X1, Y1, X2, Y2, X3, Y3: Single;
...
function TgxDelaunay2D.Triangulate(nvert: Integer): Integer;
...
procedure TgxDelaunay2D.Mesh(sort: Boolean);
begin
  if sort then
...
procedure TgxDelaunay2D.AddPoint(X, Y, Z, U, V: Single; MatIndex: Integer);
var
  i, AE: Integer;
begin
...
procedure TgxDelaunay2D.AddPointNoCheck(X, Y, Z, U, V: Single; MatIndex: Integer);
begin
  Vertex[tPoints].X := X;
  Vertex[tPoints].Y := Y;
  Vertex[tPoints].Z := Z;
...
procedure TgxDelaunay2D.RemoveLastPoint;
begin
  tPoints := tPoints - 1;
end;

procedure TgxDelaunay2D.QuickSort(var A: TDVertex; Low, High: Integer);
